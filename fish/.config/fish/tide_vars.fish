# tide_vars.fish — snapshot of tide (prompt theme) universal variables.
#
# Applied ONCE by ~/.dotfiles/bootstrap.sh on fresh machines (universal
# variables persist in ~/.config/fish/fish_variables afterwards, so this
# file is NOT auto-sourced on every shell startup — re-setting them each
# launch would make interactive tweaks impossible).
#
# To re-apply manually:   fish -c 'source ~/.config/fish/tide_vars.fish'
# To regenerate after changing your prompt:  fish ~/.dotfiles/scripts/dump-tide.fish > ~/.dotfiles/fish/.config/fish/tide_vars.fish

set -U tide_aws_bg_color 444444
set -U tide_aws_color FF9900
set -U tide_aws_icon 
set -U tide_bun_bg_color 444444
